...
export default function DownloadPortal({ onOpenAuthModal }) {
  ...
  const handleTriggerDownload = (item) => {
    const content = `NetGuard AI Attack Forecasting Client v1.0.4 (NTRO PS-26153)\nTarget: ${item.name}\nSHA-256: ${item.sha256}\nAir-Gapped Operational Binary.`;
    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = item.file.endsWith('.exe') || item.file.endsWith('.tar.gz') ? item.file : `${item.id}-setup-netguard.txt`;
